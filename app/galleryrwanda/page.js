...
export default function galleryrwanda() {
 ...
 const deletePost = async (id) => {
  ...
  if (confirmDelete) {
    try {
      const response = await axios.delete(`http://127.0.0.1:8000/pictures/deleter/${id}`, {
        headers: {
      ...
      if (response.status === 204) {
        ...
        const updatedImagesResponse = await axios.get('http://127.0.0.1:8000/pictures/imagesr/', {
          headers: {
            'Authorization': `Token ${token}`,
          },
        });
